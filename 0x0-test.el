...
(defun 0x0-test--call-service (server)
  ...
  (let* ((server (cdr (assq server-key 0x0-servers)))
    ...
    (ert-set-test name
                  (make-ert-test
                   :name name
                   :tags '(:integration)
                   :body (lambda () (0x0-test--call-service server))))))

;; Developer unit tests
(ert-deftest 0x0-test--calculate-timeout ()
...
(ert-deftest 0x0-test--unit-send-fallback-to-url ()
...
(ert-deftest 0x0-test--use-curl-if-installed ()
  (cl-letf (((symbol-function 'executable-find) (lambda (cmd)
                                                  (should (equal cmd "curl"))))
...
(ert-deftest 0x0-test--handle-response-success ()
  (let ((server (list
                 :scheme "https"
                 :host "example.com"
                 :curl-args-fun #'ignore))
        (resp-txt "Some garbage nobody cares about. But, the good stuff is below.

https://example.com/abcdefghi.jpg

Other stuff."))
    (with-temp-buffer
      (rename-buffer "*resp test*")
      (insert resp-txt)
      (should
       (equal (0x0--handle-resp server 1 (current-buffer))
              "https://example.com/abcdefghi.jpg"))
      (should (equal (current-kill 0) "https://example.com/abcdefghi.jpg"))
      (should-not (buffer-live-p "*resp test*")))))

(ert-deftest 0x0-tst--handle-response-failure ()
  (let ((server (list
                 :scheme "https"
                 :host "example.com"
                 :curl-args-fun #'ignore))
        (resp-txt "Some garbage nobody cares about."))
    (with-temp-buffer
      (rename-buffer "*resp test*")
      (insert resp-txt)
      (should-error (0x0--handle-resp server 1 (current-buffer))
                    :type 'error)
      (should (equal (buffer-name) "*resp test*")))))
